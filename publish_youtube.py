"""
YouTube Upload & Thumbnail Publishing Module for Chakra Healing
Uses OAuth 2.0 refresh token to upload 1-hour 1080p HD Chakra Meditation videos and set custom thumbnails.
"""
import os
import sys
import time
...
def get_authenticated_service():
    ...
    if not all([client_id, client_secret, refresh_token]) or "your_" in client_id:
        raise ValueError(
            "Missing YouTube credentials! Set YT_CLIENT_ID, YT_CLIENT_SECRET, and YT_REFRESH_TOKEN in .env"
        )
...
def upload_to_youtube(video_path, title, description, tags=None, category_id='10', privacy_status='public'):
    ...
    if tags is None:
        tags = [
            'ChakraHealing', '7Chakras', 'SolfeggioFrequencies', '432Hz', '528Hz',
            'KundaliniAwakening', 'MeditationMusic', 'AuraCleanse', 'DeepHealing', 'Prana'
        ]
